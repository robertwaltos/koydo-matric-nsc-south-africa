...
import { emitPrismSignal } from "@/lib/matric-nsc-south-africa/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "matric-nsc-south-africa" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "matric-nsc-south-africa" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "matric-nsc-south-africa" });
